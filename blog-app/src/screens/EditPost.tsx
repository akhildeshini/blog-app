...
const EditPost=()=>{
    ...
    const loginMuatation=useMutation({
        ...
        mutationFn:async(PostDetails:{
            ...
            const res=await authAxios.put(`http://localhost:5000/posts/${id}`,PostDetails)
            const data=res.data;
            return data;
        },
        onSuccess:()=>{
    ...
    if(isLoading)
    {
        return <Box sx={{height:'100vh',width:'100vw',justifyContent:'center',alignItems:'center'}}>
            <CircularProgress/>
        </Box>
    }
    ...
    <TextField variant='outlined' type="text" placeholder="image" sx={{marginBottom:'1rem',width:'100%'}} onChange={(e)=>setimage(e.target.value)} value={image} onBlur={()=>validate(image,'image')}/>
       ...
       <Box sx={{display:'flex'}}>
        <Button variant="contained"  sx={{width:'300px',marginLeft:'auto',marginRight:'auto'}} onClick={()=>navigate('/dashboard')}>Cancel</Button>
